use ratatui::{layout::{Alignment, Constraint, Direction, Layout, Rect}, style::{Color, Modifier, Style, Stylize}, text::Line, widgets::{Block, Borders, Paragraph}, Frame};

use super::joinroom_textarea::JoinRoomTextArea;
...
pub fn draw_joinroom_panel(
    frame: &mut Frame,
    area: Rect,
    joinroom_ta: &mut JoinRoomTextArea
) {

    let joinroompanel_block = Block::default()
            .title("Join a room")
    ...
    frame.render_widget(&joinroom_ta.roomname_ta, ta_chunks[0]);
    frame.render_widget(&joinroom_ta.roomkey_ta, ta_chunks[1]);
    ...
    let submit_paragraph = Paragraph::new("[Enter]Join".white().bold())
    ...
    frame.render_widget(&joinroom_ta.status_block, status_area);

    frame.render_widget(joinroompanel_block, area);
