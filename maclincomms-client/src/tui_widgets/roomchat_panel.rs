...
pub fn draw_roomchat_panel(
    frame: &mut Frame,
    area: Rect,
    roomchatcomps: &mut RoomChatComponents
) {

    let chatpanel_block = Block::default()
            .title(roomchatcomps.room_name.clone())
    ...
    let chat_content: Vec<Text> = {
        ...
        let chat_history = roomchatcomps.chat_history.lock().unwrap(); // Lock before iterating
...
// LOOP FOR MAKING CHAT BUBBLES -------
    let chat_history = roomchatcomps.chat_history.lock().unwrap();
    ...
    for mesg_parts in chat_history.iter() {
        ...
        // YOUR CHAT BUBBLE
        if username==roomchatcomps.username && is_join_leave_msg==false{
        ...
        else if username=="maclincomms".to_owned() && is_join_leave_msg==false{
        ...
        else if username!=roomchatcomps.username && username!="maclincomms".to_owned() && is_join_leave_msg==true{
    ...
    frame.render_widget(&roomchatcomps.input_ta, input_chunk);

    frame.render_stateful_widget(chats_scroll_view, chat_chunk, &mut roomchatcomps.scroll_state);
...
#[derive (Debug, Clone)]
pub struct RoomChatComponents {
    pub input_ta: TextArea<'static>,
    pub scroll_state: ScrollViewState,
    pub chat_history: Arc<Mutex<Vec<(String, Text<'static>, String, bool, String)>>>,  // Vector of tuple of (username, his message, time, is_join_message, ack(>))
    pub username: String,
    pub room_name: String,
    pub room_key: String
}

impl RoomChatComponents {
    ...
    pub fn get_input_textarea() -> TextArea<'static> {
        ...
        ta.set_placeholder_text("Chat with your friends");
        ...
        ta.set_block(
            Block::default()
                ...
                .title_top(Line::from("Try Whisper Mode => whisper --hf [username1,username2] <Your Message>").right_aligned().light_cyan().on_black()),
        );
