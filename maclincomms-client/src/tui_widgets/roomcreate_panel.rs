...
use super::roomcreation_textarea::RoomCreationTextArea;


pub fn draw_roomcreate_panel(
    frame: &mut Frame,
    area: Rect,
    roomcreation_ta: &mut RoomCreationTextArea
) {

    let roomcreatepanel_block = Block::default()
            .title("Create a room")
    ...
    .constraints(
        [
            Constraint::Length(3), // roomname field
    ...
    frame.render_widget(&roomcreation_ta.roomname_ta, ta_chunks[0]);
    ...
    let submit_paragraph = Paragraph::new("[Enter]Create".white().bold())
    ...
    frame.render_widget(&roomcreation_ta.status_block, status_area);

    
    frame.render_widget(roomcreatepanel_block, area);
