...
use ratatui::{layout::{Alignment, Constraint, Direction, Layout, Margin, Rect, Size}, style::{Color, Modifier, Style, Stylize}, text::{Line, Text, ToLine}, widgets::{Block, Borders, Paragraph, Wrap}, Frame};
...
pub fn draw_dmchat_panel(
    frame: &mut Frame,
    area: Rect,
    dmchatcomps: &mut DmChatComponents
) {

    let chatpanel_block = Block::default()
            .title(dmchatcomps.to_user.clone())
    ...
    let chat_content: Vec<Text> = {
        ...
        let chat_history = dmchatcomps.chat_history.lock().unwrap(); // Lock before iterating

        chat_history.iter()
            .map(|(_, text, _, _, _)| text.clone()) // Extract the `Line` from each tuple
            .collect()
    };
...
// LOOP FOR MAKING CHAT BUBBLES -------
    let chat_history = dmchatcomps.chat_history.lock().unwrap();
    ...
    for mesg_parts in chat_history.iter() {
        ...
        let is_online_offline_msg = mesg_parts.3;

        let message_ack = mesg_parts.4.as_str();

        // YOUR CHAT BUBBLE
        if username==dmchatcomps.username && is_online_offline_msg==false {
        ...
        //MACLINCOMMS CHAT BUBBLE
        else if username=="maclincomms".to_owned() && is_online_offline_msg==false {
        ...
        else if username!=dmchatcomps.username && username!="maclincomms".to_owned() && is_online_offline_msg==true{
    ...
    frame.render_widget(&dmchatcomps.input_ta, input_chunk);

    frame.render_stateful_widget(chats_scroll_view, chat_chunk, &mut dmchatcomps.scroll_state);
...
//ROOM CHAT COMPONENETS

#[derive (Debug, Clone)]
pub struct DmChatComponents {
    pub input_ta: TextArea<'static>,
    pub scroll_state: ScrollViewState,
    pub chat_history: Arc<Mutex<Vec<(String, Text<'static>, String, bool, String)>>>,  // Vector of tuple of (username, his message, time, is_online_offline, message ack(> or >>))
    pub username: String,
    pub to_user: String,
}

impl DmChatComponents {
    ...
    pub fn get_input_textarea() -> TextArea<'static> {
        ...
        ta.set_placeholder_text("What's good");
